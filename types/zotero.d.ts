declare namespace Zotero {
  ...
  interface Item extends DataObject {
    ...
    /**
     * Annotation fields. Only meaningful on annotation items (isAnnotation()).
     * `annotationText` is populated for 'highlight'/'underline' types only.
     */
    ...
    readonly annotationSortIndex: string;
    ...
    /**
     * Annotation items on a file attachment, in stored order.
     * Throws if called on an item that is not a file attachment.
     */
    getAnnotations(includeTrashed?: boolean): Item[];
  ...
  interface ItemTypes extends CachedTypes {
    ...
    getTypes(): { id: number; name: string }[];
  }
