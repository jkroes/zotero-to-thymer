  ...
  async reconcileAnnotations(refGuid, annos) {
    ...
    for (const a of annos) {
      ...
      if (!guid) {
        ...
        if (!guid) continue;
        // In-tick hydration gap: a freshly created record's props aren't resolvable
        // (and writes to them are lost) until the property map hydrates — poll before
        // writing (verified live 2026-07-03).
        rec = null;
